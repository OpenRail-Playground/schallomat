<template>
  <div>
    ...
    <p v-if="!addressStore.loading && filteredAndSortedAddresses.length === 0">No addresses found.</p>
  </div>
</template>
...
const filteredAndSortedAddresses = computed(() => {
  return addressStore.addresses
    .filter((address) => {
      ...
      return (
        [address.street, address.housenumber, address.postcode, address.city].join(" ").toLowerCase().includes(searchLower) ||
